...
/// extra messages.
/// {@endtemplate}
class OllamaFunctionsAgent extends BaseSingleActionAgent {
  /// {@macro openai_functions_agent}
  OllamaFunctionsAgent({
    required this.llmChain,
    required super.tools,
  })  : _parser = const OllamaFunctionsAgentOutputParser(),
        assert(
          llmChain.memory != null || llmChain.prompt.inputVariables.contains(BaseActionAgent.agentScratchpadInputKey),
  ...
  final LLMChain<ChatOllama, ChatOllamaOptions, void, BaseChatMemory> llmChain;

  /// Parser to use to parse the output of the LLM.
  final OllamaFunctionsAgentOutputParser _parser;
  ...
  factory OllamaFunctionsAgent.fromLLMAndTools({
    required final ChatOllama llm,
    required final List<BaseTool<ToolOptions>> tools,
    final BaseChatMemory? memory,
    final SystemChatMessagePromptTemplate systemChatMessage = _systemChatMessagePromptTemplate,
    final List<ChatMessagePromptTemplate>? extraPromptMessages,
  }) {
    return OllamaFunctionsAgent(
      llmChain: LLMChain(
        llm: llm,
        llmOptions: ChatOllamaOptions(
          functions: tools.map((final t) => t.toChatFunction()).toList(growable: false),
        ),
        prompt: createPrompt(
  ...
  Map<String, dynamic> _constructLlmChainInputs(
    ...
    return {
      ...
      if (llmChain.memory == null) BaseActionAgent.agentScratchpadInputKey: _constructScratchPad(intermediateSteps),
    };
  }
  ...
  static BasePromptTemplate createPrompt({
    ...
    return ChatPromptTemplate.fromPromptMessages([
      systemChatMessage,
      ...?extraPromptMessages,
      for (final memoryKey in memory?.memoryKeys ?? {}) MessagesPlaceholder(variableName: memoryKey),
...
/// {@endtemplate}
class OllamaFunctionsAgentOutputParser extends BaseLLMOutputParser<AIChatMessage, ChatOllamaOptions, List<BaseAgentAction>> {
  /// {@macro openai_functions_agent_output_parser}
  const OllamaFunctionsAgentOutputParser();
